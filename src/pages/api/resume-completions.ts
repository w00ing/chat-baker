...
import { LLMChain, OpenAI, Prompt } from "@promptable/promptable";
...
export default async function handler(
  ...
  const generateBulletPrompt = new Prompt(
    `Write 3 resume professional "\u2022" points for a {{title}} at {{organization}}. 
    If relevant to the position one should be quanitifiable, do not label it as quanitifiable.`,
    ["title", "organization"]
  );
  ...
  const bullets = await bulletChain.run({
    title: title,
